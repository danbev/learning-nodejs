...
Remember, `obj` is of type BaseObject and is not managed by V8, but the persistent_handle_
is managed by V8's GC. When we get the callback that the persistent_handle_ is 
about to be freed. If we don't call `Reset`, then `~BaseObject` will try to set
the internal field on the now freed `persistent_handle_`. `object()` calls and
`SetAlignedPointerInInternalField` will segfault as the handle has already been freed.
```c++
...
NativeModuleLoader::InitializeModuleCategories we can find how various modules
in the lib directory are filtered, or really not included depending on configuration
parameters.
```c++
...
### Inspector
If the build is configured to use V8's inspector protocol the following gyp file
will be include:
```console
